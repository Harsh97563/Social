...
function Comment({data}: {data:CommentData}) {
  return (
    <div className='flex flex-col w-full text-black p-3 bg-[#384B70] rounded-sm px-4 space-y-2 border-2 border-black'>
        <Link href={`/profile?username=${data.user.username}`} className='flex items-center space-x-2'>
        ...
        <div className='w-full bg-[#7E60BF] shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] p-4 rounded-sm border border-black '>{data.comment}</div>
    </div>
  )
}
